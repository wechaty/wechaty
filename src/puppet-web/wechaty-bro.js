...
(function (port) {
  ...
  */
  ...
  function emit(event, data) {
    ...
    if (bufLen) {
      if (bufLen > 1) {
        clog('WechatyBro.vars.eventsBuf has ' + bufLen + ' unsend events');
      }
      ...
      if (bufLen > 1) {
        clog('WechatyBro.vars.eventsBuf[' + bufLen + '] all sent');
      }
    }
  }
  ...
  */
  ...
  function angularIsReady() {
    ...
    return !!(typeof angular !== 'undefined' && angular.element && angular.element('body') && angular.element(document).injector());
  }
  ...
  function checkScan() {
    ...
    if (url && code !== WechatyBro.vars.scanCode) {
      ...
      log('checkScan() - code change detected: from ' + WechatyBro.vars.scanCode + ' to ' + code);
  ...
  function ding(data) {
    log('recv ding');return data || 'dong';
  }
  function hookEvents() {
  ...
  function connectSocket() {
    ...
    socket.on('connect', function (e) {
      clog('connected to server:' + e);
    });
    socket.on('disconnect', function (e) {
      clog('socket disconnect:' + e);
    });
  }
  ...
  function getContact(id) {
    ...
    if (c) {
       ...
       */
      c = Object.keys(_contacts).filter(function (id) {
        return id.match(/^@@/);
      }) // only search in room
      .map(function (id) {
        return _contacts[id];
      ...
      .filter(function (r) {
        return r.MemberList.length;
      }) // get rid of room without member list
      .filter(function (r) {
        return r.MemberList.filter(function (m) {
          return m.UserName === id;
        }).length;
      }).map(function (c) {
        return c.MemberList.filter(function (m) {
          return m.UserName === id;
        })[0];
      })[0];
  ...
  function getUserName() {
    var accountFactory = WechatyBro.glue.accountFactory;
    return accountFactory ? accountFactory.getUserName() : null;
  }
  ...
  function contactFindAsync(filterFunction) {
    ...
    function retryFind() {
      ...
      var contactList = contactFactory.getAllFriendContact().filter(function (c) {
        return match(c.NickName);
      }).map(function (c) {
        return c.UserName;
      });
  ...
  function roomFind(filterFunction) {
    ...
    // log(match.toString())
    return contactFactory.getAllChatroomContact().filter(function (r) {
      return match(r.NickName);
    }).map(function (r) {
      return r.UserName;
    });
  }
  ...
  function roomCreateAsync(UserNameList, topic) {
    ...
    var UserNameListArg = UserNameList.map(function (n) {
      return { UserName: n };
    });

    var chatroomFactory = WechatyBro.glue.chatroomFactory;
    var state = WechatyBro.glue.state;
    chatroomFactory.create(UserNameListArg).then(function (r) {
      if (r.BaseResponse && 0 == r.BaseResponse.Ret || -2013 == r.BaseResponse.Ret) {
        ...
        throw new Error('chatroomFactory.create() error with Ret: ' + r && r.BaseResponse.Ret + 'with ErrMsg: ' + r && r.BaseResponse.ErrMsg);
      }
    }).catch(function (e) {
  ...
  function verifyUserRequest(UserName) {
    var VerifyContent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
