...
test('padchatDecode()', async t => {
  t.test('uri decode with +', async t => {
...
test('messageRawPayloadParser', async t => {
  ...
  t.test('sys', async t => {
    ...
    const payload = pfHelper.messageRawPayloadParser(PADCHAT_MESSAGE_PAYLOAD_SYS)
    // console.log('payload:', payload)
    t.deepEqual(payload, EXPECTED_MESSAGE_PAYLOAD_SYS, 'should parse sys message payload')
  })

  t.test('status notify', async t => {
    const PADCHAT_MESSAGE_PAYLOAD_STATUS_NOTIFY: PadchatMessagePayload = {
      content:
    ...
    const EXPECTED_MESSAGE_PAYLOAD_STATUS_NOTIFY: MessagePayload = {
